...
public class EmployeeController {
	...
	}// End of billRevenueDetails()
	
	@GetMapping("/monthlyRevenue")
	public String getMonthlyRevenue(HttpSession session,ModelMap modelMap) {
		EmployeeMasterBean empMasterBean = (EmployeeMasterBean) session.getAttribute("masterInfo");
		if(empMasterBean != null) {
			modelMap.addAttribute("paidBillDetails", service.getPaidBillsDetails(empMasterBean.getRegion()));
			return "monthlyRevenue";
		} else {
			modelMap.addAttribute("errMsg","Unable to process the details, Please Login again");
			return "employeeLogin";
		}
	}//End of getMonthlyRevenue()
	
	@GetMapping("/customBillPay")
	public String displayBillPage() {
		return "customBillPayment";
	}//End of displayBillPage
	
	
	@PostMapping("/customBillPayment")
	public String billPayment(ModelMap modelMap, HttpSession session, String rrNumber, 
			int billAmount) {
		EmployeeMasterBean empBean =  (EmployeeMasterBean) session.getAttribute("masterInfo");
		if(empBean != null) {
			Date date = new Date();
			boolean payment = consumerService.payment(rrNumber, date, billAmount);
			if(payment == true) {
				modelMap.addAttribute("msg", "Payment done Successfully!");
				return "#";
			} else {
				modelMap.addAttribute("errMsg", "Payment Process Failed!");
				return "#";
			}
		}else {
